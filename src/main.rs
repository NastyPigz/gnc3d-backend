...
type PeerMap = Arc<Mutex<HashMap<SocketAddr, Player>>>;
...
async fn handle_connection(
    ...
    let mut plock = peer_map.lock().await;
    plock.insert(addr, Player {
        sender: tx,
        username: username.clone()
    });

    for recp in plock.iter().filter(|(peer_addr, _)| peer_addr != &&addr).map(|(_, player)| player) {
        let mut v = vec![PLAYER_JOIN];
        v.extend_from_slice(username.as_bytes());
        recp.sender.unbounded_send(Message::Binary(v)).unwrap();
    }
    ...
    let broadcast_incoming = incoming.try_for_each(|msg| {
        ...
        async move {
            ...
            let broadcast_recipients =
                peers.iter().filter(|(peer_addr, _)| peer_addr != &&addr).map(|(_, player)| player);
